...
def paused(gameDisplay,display_width,display_height,message,pause=False):
    ...
    while pause:
        ...
        quitAction = buttons.button(gameDisplay,"Quit", 750, 250, 100, 50, red, bright_red,'close_game')

        if continueAction == 'continueGame' :
            pause = False

        if quitAction == 'closeGameClicked': #'quitGame'
            pygame.quit()
